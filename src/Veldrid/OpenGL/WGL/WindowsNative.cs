...
namespace Veldrid.OpenGL.WGL
{
    internal static unsafe class WindowsNative
    {
        ...
        public static WindowsExtensionCreationFunctions GetExtensionFunctions()
        {
            lock (s_functionsLock)
            {
                if (s_functions == null)
                {
                    ...
                    IntPtr hglrc = CreateContextRegular(hdc, 0, 0);
        ...
        public static IntPtr CreateContextRegular(IntPtr hdc, uint depthBits, uint stencilBits)
        {
            ...
            if (format == 0)
            {
                throw new VeldridException("Failed to choose OpenGL pixel format.");
            }
            int setFormatResult = SetPixelFormat(hdc, format, &pfd);
            if (setFormatResult == 0)
            {
                throw new VeldridException("Failed to set OpenGL pixel format.");
            }

            IntPtr glContext = wglCreateContext(hdc);
            if (glContext == IntPtr.Zero)
            {
                throw new VeldridException("Failed to create dummy OpenGL context.");
            }
            return glContext;
        }

        public static IntPtr CreateContextWithExtension(
            ...
            uint stencilBits)
        {
            ...
            return extensionFuncs.CreateContextAttribsARB(hdc, IntPtr.Zero, contextAttribs);
        }
